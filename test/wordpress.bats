...
@test "Check that site shows up in $KBOX list with correct properties" {
  ...
  $KBOX list | grep "\"url\": \"http://${PHP_WORDPRESS_NAME}.kbox.host\""
...
@test "Check that we have the correct DNS entry." {
  $DOCKER exec kalabox_proxy_1 redis-cli -p 8160 lrange frontend:http://${PHP_WORDPRESS_NAME}.kbox.host 0 5
}
